...
OpenAI function-calling format. Used when plannerAgentMode=True.
"""
...
# Planner skills root: MAARS_PLANNER_SKILLS_DIR env or backend/planner/skills/
_PLANNER_SKILLS_DIR = os.environ.get("MAARS_PLANNER_SKILLS_DIR")
PLANNER_SKILLS_ROOT = (
    Path(_PLANNER_SKILLS_DIR).resolve()
    if _PLANNER_SKILLS_DIR
...
def _planner_list_skills() -> str:
    ...
    try:
        if not PLANNER_SKILLS_ROOT.exists() or not PLANNER_SKILLS_ROOT.is_dir():
            return orjson.dumps([]).decode("utf-8")
        skills = []
        for item in sorted(PLANNER_SKILLS_ROOT.iterdir()):
            if not item.is_dir():
                continue
...
def _planner_load_skill(name: str) -> str:
    ...
    try:
        ...
        skill_dir = (PLANNER_SKILLS_ROOT / name.strip()).resolve()
        try:
            skill_dir.relative_to(PLANNER_SKILLS_ROOT.resolve())
        except ValueError:
            return "Error: invalid skill name"
...
def _planner_read_skill_file(skill: str, path: str) -> str:
    ...
    try:
        if not skill or ".." in skill or "/" in skill or "\\" in skill:
            return "Error: invalid skill name"
        skill_dir = (PLANNER_SKILLS_ROOT / skill.strip()).resolve()
        try:
            skill_dir.relative_to(PLANNER_SKILLS_ROOT.resolve())
        except ValueError:
            return "Error: invalid skill name"
...
async def execute_planner_tool(
    ...
    *,
    ...
    if name == "GetNextTask":
        ...
        parent_id = _get_parent_id(tid)
        siblings = [t for t in all_tasks if t.get("task_id") != tid and _get_parent_id(t.get("task_id", "")) == parent_id]
        depth = len(tid.split("_"))
        context = {
            "depth": depth,
            "ancestor_path": _get_ancestor_path(tid),
            "idea": idea,
    ...
    if name == "CheckAtomicity":
        ...
        atomicity_context = {
            "depth": depth,
            "ancestor_path": ctx.get("ancestor_path") or _get_ancestor_path(task_id),
