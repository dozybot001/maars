...
def _get_planner_agent_params(api_config: Dict[str, Any]) -> tuple:
    """Return (max_turns, temperature) for planner agent from modeConfig or defaults."""
    mode_cfg = api_config.get("modeConfig") or {}
    agent_cfg = mode_cfg.get("agent") or {}
    llm_cfg = mode_cfg.get("llm") or {}
    max_turns = agent_cfg.get("plannerAgentMaxTurns") or MAX_PLANNER_AGENT_TURNS
    max_turns = int(max_turns)
    temperature = (
        agent_cfg.get("plannerLlmTemperature")
        or llm_cfg.get("plannerLlmTemperature")
        or 0.3
    )
...
async def run_plan_agent(
    ...
    max_turns, temperature = _get_planner_agent_params(api_config or {})
...
async def run_plan(
    ...
    """Run atomicity->decompose->format from root task, top-down to all atomic tasks. When plannerAgentMode=True, uses Agent loop instead."""
    ...
    if api_config and api_config.get("plannerAgentMode"):
