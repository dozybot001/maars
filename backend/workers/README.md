...
执行阶段：**Generate Map**（生成执行图）→ **Execution**（任务执行与验证）。
...
### POST `/api/execution/generate-from-plan`
...
详见 [EXECUTOR_IMPROVEMENTS.md](EXECUTOR_IMPROVEMENTS.md)。
